...
*   **CORS (Cross-Origin Resource Sharing) Security:** Karena Backend (Railway) dan Frontend (Cloudflare) berbeda domain, browser memblokir request lintas origin kecuali Laravel mengizinkannya. Set di Railway variable `CORS_ALLOWED_ORIGINS` (contoh: `http://localhost:5173,https://cook-edu.fiksfaa.workers.dev`). `backend/config/cors.php` juga mengizinkan pola `*.workers.dev` dan `*.pages.dev` agar preview Cloudflare tetap jalan. Setelah mengubah CORS, redeploy service Laravel di Railway.
*   **PWA icons:** File `frontend/public/pwa-192x192.png` dan `pwa-512x512.png` harus ada sebelum `npm run build`; jika hilang, Cloudflare mengembalikan HTML SPA dan manifest icon gagal dimuat.
