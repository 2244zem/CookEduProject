...
import { Bookmark, BookOpen, Snowflake, Globe, User, Moon, Sun } from 'lucide-react'
  ...
  return (
    <nav className="fixed bottom-8 inset-x-0 z-50 flex justify-center px-6 pointer-events-none">
      <motion.div 
        ...
        className="bg-white/80 dark:bg-slate-850/80 backdrop-blur-3xl border border-white/60 dark:border-sky-500/10 rounded-[40px] py-2 px-6 shadow-xl flex items-center justify-between w-full max-w-sm pointer-events-auto"
      >
        {navItems.map((item) => {
            ...
            <button 
              ...
              className={`relative p-3 transition-all ${
                isActive 
                  ...
                  : "text-slate-450 hover:text-slate-700 dark:text-sky-200/50 dark:hover:text-white"
              }`}
            >
        ...
        <button
          ...
          className="relative p-3 transition-all text-slate-450 hover:text-slate-700 dark:text-sky-200/50 dark:hover:text-white active:scale-95"
        >
